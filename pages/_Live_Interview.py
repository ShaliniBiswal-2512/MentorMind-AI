...
def synthesize_question(text, q_id):
    ...
    try:
        ...
        # Handle async execution robustly with retries
        try:
        ...
        for attempt in range(3):
            try:
                ...
                print(f"TTS network attempt {attempt} failed: {e}")
                time.sleep(1)
                
        if not success:
            raise Exception("TTS failed after 3 attempts.")
...
if current_level >= 90:
    ...
    if not st.session_state.get('disqualified_saved', False):
        try:
            l1_d = getattr(st.session_state, 'l1_answers', {})
            q1 = getattr(st.session_state, 'apt_qs', [])
            l1 = int(sum(1 for i, q in enumerate(q1) if l1_d.get(i) == q['answer']) / len(q1) * 100) if q1 else 0
            
            l2_d = getattr(st.session_state, 'l2_answers', {})
            q2 = getattr(st.session_state, 'scen_qs', [])
            l2 = int(sum(1 for i, q in enumerate(q2) if l2_d.get(i) == q['answer']) / len(q2) * 100) if q2 else 0

            scores = {"clarity": 0, "communication": 0, "problem_solving": 0, "subject_knowledge": 0, "teaching_ability": 0}
            save_interview(user_id, config['subject'], config['class_level'], scores, "Disqualified", f"Auto-failed due to {reason}", st.session_state.transcript, l1_score=l1, l2_score=l2, violations_count=st.session_state.tab_switches)
        ...
        # ===================== LEVEL 3 =====================
            ...
            try:
                ...
                if audio_bytes:
                    ...
                    player_id_suffix = f"{idx}_{int(time.time() * 1000)}" if should_autoplay else idx
                    
                    st.markdown(f"""
                        <audio id="l3-audio-player-{player_id_suffix}" class="sys-audio" src="data:audio/mp3;base64,{b64_audio}" controls {auto_attr} style="width: 100%; border-radius: 12px; margin-bottom: 20px; outline: none; border: 1px solid rgba(88,166,255,0.2); background: rgba(0,0,0,0.2);"></audio>
                    """, unsafe_allow_html=True)
            ...
            if "evaluation_processing" not in st.session_state:
                st.session_state.evaluation_processing = True
                ...
                l1_score_calc = int((sum(1 for i, q in enumerate(apt_qs) if l1_data.get(i) == q['answer']) / len(apt_qs)) * 100) if apt_qs else 0
                l2_score_calc = int((sum(1 for i, q in enumerate(scen_qs) if l2_data.get(i) == q['answer']) / len(scen_qs)) * 100) if scen_qs else 0

                def _bg_eval(transcript, subject, class_lvl, summary1, summary2, uid, l1_s, l2_s):
                    from core.groq_api import evaluate_interview
                    from db.database import save_interview
                    import datetime
                    try:
                        evaluation = evaluate_interview(transcript, subject, class_lvl, summary1, summary2)
                        save_interview(
                            uid, subject, class_lvl, evaluation['scores'], 
                            evaluation['verdict'], evaluation['insights'], 
                            transcript, l1_score=l1_s, l2_score=l2_s
                        )
                        evaluation['date'] = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                        evaluation['subject'] = subject
                        evaluation['class_level'] = class_lvl
                        evaluation['l1_score'] = l1_s
                        evaluation['l2_score'] = l2_s
                ...
                import threading
                from streamlit.runtime.scriptrunner import add_script_run_ctx
                t = threading.Thread(target=_bg_eval, args=(st.session_state.transcript, config['subject'], config['class_level'], l1_summary, l2_summary, user_id, l1_score_calc, l2_score_calc))
                add_script_run_ctx(t)
                t.start()

            st.markdown("<div class='premium-card' style='text-align:center;'><h4>⏳ Transmitting secure data to AI Evaluation Matrix...</h4><p style='color:var(--text-muted);'>This process runs on cloud infrastructure and takes approximately 10-15 seconds. Please ensure you remain in frame.</p></div>", unsafe_allow_html=True)

            if "evaluation_result" in st.session_state:
                st.session_state.evaluation = st.session_state.evaluation_result
                del st.session_state.evaluation_processing
                del st.session_state.evaluation_result
                st.switch_page("pages/6_Evaluation_Insights.py")
            elif "evaluation_error" in st.session_state:
                st.error(f"Matrix sync failure: {st.session_state.evaluation_error}")
                if st.button("Retry Matrix Sync"):
                    del st.session_state.evaluation_processing
                    del st.session_state.evaluation_error
                    st.rerun()